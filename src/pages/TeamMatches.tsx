...
const formatScore = (match: Match) => {
  ...
  let score = (homeFt !== undefined && homeFt !== null && awayFt !== undefined && awayFt !== null)
    ? `${homeFt}:${awayFt}`
    : '?:?';
...
export default function TeamMatches() {
  ...
  useEffect(() => {
    ...
    getTeamById(teamId)
      ...
      .catch(() => setTeamName(`Команда ${teamId}`));
  }, [teamId]);

  useEffect(() => {
    if (!teamId) return;
    setLoading(true);
    getTeamMatches(teamId, dateFrom || undefined, dateTo || undefined)
      .then(data => {
        setMatches(data || []);
        setError(null);
      })
  ...
  if (loading) return <div style={{ padding: '20px' }}>Загрузка матчей команды...</div>;
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '20px' }}>
        <Link to="/teams">Команды</Link> &gt; <span>{teamName}</span>
      </div>
      <h1>Календарь команды</h1>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <label>
          Дата с:
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} style={{ marginLeft: '5px', padding: '5px' }} />
        </label>
        <label>
          Дата по:
          <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} style={{ marginLeft: '5px', padding: '5px' }} />
        </label>
